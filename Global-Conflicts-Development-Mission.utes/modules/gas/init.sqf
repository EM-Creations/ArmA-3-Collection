...
if (!isDedicated && hasInterface) then {
	[{!isNull player}, {
		...
		[] spawn {
			...
			_maskOn = false;
			while {true} do {
				if (goggles player in GVAR(MASKLIST) && !_maskOn) then {
					_maskOn = true;
		...
		_maskUpFriend = ["maskUp_class", "Put Gasmask On", "", {
			...
			([_player, _target, []] call ace_common_fnc_canInteractWith) && ({_x in items _target} count GVAR(MASKLIST) > 0 || {_x in items _player} count GVAR(MASKLIST) > 0) && !(goggles _target in GVAR(MASKLIST)) && _target getVariable ["FW_GAS_NEEDSMASK",false]
		}] call ace_interact_menu_fnc_createAction;
