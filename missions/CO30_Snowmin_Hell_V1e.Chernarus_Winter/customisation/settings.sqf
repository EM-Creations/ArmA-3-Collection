...
if (isServer) then {
	...
	FW_TimeLimitMessage = "TIME LIMIT REACHED - BLAVAKS VICTORY!"; //The message displayed when the time runs out

	[resistance, "BLAVAKS", "player"] call FNC_AddTeam; // Adds a player team called BLUFOR on side blufor
	[opfor, "ROYAL", "ai"] call FNC_AddTeam; // Adds a ai team called OPFOR on side opfor
